...
import { TbGenderFemme, TbGenderMale } from "react-icons/tb";
import { MdSportsKabaddi, MdOutlineChildFriendly } from "react-icons/md";
import { GiCrystalShrine, GiQueenCrown } from "react-icons/gi";
    ...
    <div className="explore py-32 bg-slate-100 w-full h-full flex items-center justify-center flex-col">
      ...
      <div className="w-fit bg-white text-base font-medium rounded-full mt-10 flex p-2 space-x-3 shadow-lg">
        {data.map((item) => (
          <button
            key={item.name}
            className={`lg:px-4 lg:py-2 px-2 py-1 flex items-center rounded-full cursor-pointer ${tabActive === item.name
                ? "text-white bg-black"
        ...
        {cardData.map((item) => (
          ...
          >
            ...
            <div className="flex items-center justify-between gap-32">
              ...
              />
              <h1 className="text-neutral-700 text-sm">{item.count} products</h1>
            </div>
